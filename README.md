...
- **A separate Agent cursor:** click, scroll, and drag actions animate a click-through, nonactivating software cursor while the macOS system cursor remains untouched. It appears only while the exact target application is frontmost, follows a speed/acceleration-shaped curved path, and completes its move before input is delivered.
