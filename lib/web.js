...
/**
 * Whether one thrown value is the Settings provider's optimistic-concurrency
 * conflict. The provider — not this module — throws it, so a second copy of
 * `SettingsConflictError` (plugin-local versus host-local install) makes
 * `instanceof` alone unreliable. Both the 0.1.5 and the older class carry the
 * stable machine code `"SETTINGS_CONFLICT"`, which is checked first-class;
 * `instanceof` stays as an equivalent additional branch.
 */
