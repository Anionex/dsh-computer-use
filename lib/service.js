...
export class ComputerUseService extends Service {
    ...
    async act(action, context) {
        ...
        // The settle loop already decides whether the target changed; reporting it
        // is what lets a caller tell a real effect from a no-op. Without this a
        // drag that moved nothing is indistinguishable from one that worked, since
        // `pointerRouting: target-process` only describes what was attempted.
        ...
        return {
            ...
            // Always present, because "did anything happen" is the question every
            // action needs answered. `false` is not an error: some actions are
            // legitimately idempotent, and some surfaces (a Chromium canvas, a
            // window whose movement the window server owns) change nothing this
            // observation can see. It is a fact the caller must weigh, not a verdict.
            effect: {
                targetChanged: settled,
                observedForMs: Date.now() - started,
                ...(settled ? {} : {
                    note: 'nothing changed in the window title, id, frame, or element tree after this action;'
                        + ' a window move or resize would have shown here, so this is evidence the action had no effect,'
                        + ' not merely that accessibility could not see it',
                }),
            },
    ...
    async wait(stored, action, context, signal) {
        ...
        return {
            ...
            // A wait that returns rather than timing out did so because its condition
            // became true, which is a change by definition.
            effect: { targetChanged: true, observedForMs: timeoutMs - Math.max(0, deadline - Date.now()) },
            pointerInput: false,
            pointerRouting: 'none',
            observation,
        };
